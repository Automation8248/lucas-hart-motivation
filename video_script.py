...
def get_real_nature_img():
    ...
    img_url = random.choice(res.json()["hits"])["largeImageURL"]
...
def get_soft_piano_music():
    ...
    audio = requests.get(sound["previews"]["preview-hq-mp3"]).content
...
def create_video(quote):
    ...
    audio = AudioFileClip(music).subclip(0, DURATION).volumex(0.25)
